...
import { cleanHeadline, cleanQuote, hasValue } from '@/lib/format';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <ProfileSection title="Hero Headline">
              ...
              {hasValue(s.deepPrimaryMessage) && (
                <div style={{ background: 'var(--parchment-mid)', borderRadius: 'var(--radius)', padding: '12px 14px', marginBottom: '1rem' }}>
                  <div style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--ink-light)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Primary Message</div>
            ...
            <ProfileSection title="Messaging Analysis">
              {hasValue(s.deepKeyPhrases) && (
                <div style={{ background: 'var(--parchment-mid)', borderRadius: 'var(--radius)', padding: '12px 14px', marginBottom: '1rem' }}>
                  <div style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--ink-light)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' }}>Key Phrases</div>
                  <p style={{ fontSize: '0.95rem', fontFamily: 'var(--font-display)', fontStyle: 'italic', color: 'var(--navy)', lineHeight: 1.6 }}>{cleanQuote(s.deepKeyPhrases)}</p>
                </div>
              )}
              <DataRow label="Strongest phrase" value={cleanQuote(s.strongestPhrase)} verbatim />
              <DataRow label="Weakest pattern" value={s.weakestPatternIdentified} />
              <DataRow label="CTA labels" value={cleanQuote(s.ctaLabels)} verbatim />
              {hasValue(s.deepCtaLabels) && <DataRow label="Page CTAs (deep)" value={cleanQuote(s.deepCtaLabels)} verbatim />}
            ...
            <ProfileSection title="Visual Theology">
              {hasValue(s.deepVisualTheologyNote) && (
                <div style={{ background: 'var(--parchment-mid)', borderRadius: 'var(--radius)', padding: '12px 14px', marginBottom: '1rem' }}>
                  <p style={{ fontSize: '0.95rem', fontFamily: 'var(--font-display)', fontStyle: 'italic', color: 'var(--navy)', lineHeight: 1.65 }}>{s.deepVisualTheologyNote}</p>
                </div>
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ background: 'var(--white)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }}>
              <div style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--red)', marginBottom: '1rem' }}>Strength signals</div>
            ...
            <div style={{ background: 'var(--navy)', borderRadius: 'var(--radius-lg)', padding: '1.5rem', textAlign: 'center' }}>
              <div style={{ fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-faint)', marginBottom: '8px' }}>Want more?</div>
              <p style={{ fontFamily: 'var(--font-display)', color: '#fff', fontSize: '1.1rem', marginBottom: '8px' }}>Unlock all pages & export</p>
            ...
            {/* School info */}
            <div style={{ background: 'var(--white)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }}>
              <div style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--red)', marginBottom: '1rem' }}>School info</div>
...
function ProfileSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: 'var(--white)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.75rem' }}>
      <div style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--red)', marginBottom: '1.25rem' }}>{title}</div>
      {children}
    </div>
  );
}
