  ...
  return (
    <div style={{ minHeight: '100vh' }}>
        ...
        <div className="container">
          ...
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
            {tabs.map(tab => (
              <a
                key={tab.id}
                href={tab.comingSoon ? '/pricing' : `/school/${params.slug}?tab=${tab.id}`}
                style={{
                  ...
                  color: activeTab === tab.id ? 'var(--navy)' : tab.hasData ? 'var(--parchment-dark)' : 'rgba(255,255,255,0.35)',
                  textDecoration: 'none',
                  border: activeTab === tab.id ? '1px solid var(--border)' : '1px solid transparent',
                  borderBottom: activeTab === tab.id ? '1px solid var(--parchment)' : '1px solid transparent',
                  cursor: tab.hasData || tab.comingSoon ? 'pointer' : 'default',
                }}
              >
                {tab.label}
                {!tab.hasData && !tab.comingSoon && (
                  <span style={{ fontSize: '0.6rem', opacity: 0.5 }}>—</span>
                )}
                {tab.comingSoon && (
                  <span style={{ fontSize: '0.6rem', background: 'var(--red)', color: '#fff', padding: '1px 4px', borderRadius: '3px' }}>Soon</span>
                )}
              </a>
            ))}
          </div>
      ...
      <div className="container" style={{ padding: '2.5rem 2rem' }}>
        ...
        {/* ── FULL SITE TAB ─────────────────────────────────── */}
        {activeTab === 'fullsite' && fullSite && <FullSiteTab data={fullSite} />}
        {activeTab === 'fullsite' && !fullSite && <LockedTab label="Full Site" message="Full site analysis is coming for this school. Check back soon or submit a request." />}

        {/* ── ADMISSIONS TAB ────────────────────────────────── */}
        {activeTab === 'admissions' && admissions && <AdmissionsTab admissions={admissions} slug={params.slug} />}
        {activeTab === 'admissions' && !admissions && <LockedTab label="Admissions" message="Admissions page analysis is coming for this school. More schools are added regularly." />}

        {/* ── STUDENT LIFE TAB ──────────────────────────────── */}
        {activeTab === 'studentlife' && studentLife && <StudentLifeTab data={studentLife} />}
        {activeTab === 'studentlife' && !studentLife && <LockedTab label="Student Life" message="Student life analysis is coming for this school." />}

        {/* ── ACADEMICS TAB ─────────────────────────────────── */}
        {activeTab === 'academics' && academics && <AcademicsTab data={academics} />}
        {activeTab === 'academics' && !academics && <LockedTab label="Academics" message="Academics analysis is coming for this school." />}

        {/* ── LINKEDIN TAB ──────────────────────────────────── */}
        {activeTab === 'linkedin' && <LockedTab label="LinkedIn" message="LinkedIn content analysis is coming soon. Upgrade to be notified when it launches." />}
...
function FullSiteTab({ data }: { data: FullSiteAnalysis }) {
  return (
    <div className="profile-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem', alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {hasValue(data.narrative) && (
          <NavyCard title="Full Site Narrative">
            <p style={{ fontSize: '1.05rem', color: 'var(--parchment)', lineHeight: 1.8 }}>{data.narrative}</p>
          </NavyCard>
        )}
        {hasValue(data.whatToSteal) && (
          <NavyCard title="What to Steal" accent>
            <p style={{ fontSize: '1rem', color: 'var(--parchment)', lineHeight: 1.75 }}>{data.whatToSteal}</p>
          </NavyCard>
        )}
        ...
        <ProfileSection title="Pages Evaluated">
          <DataRow label="Pages reviewed" value={data.pagesEvaluated} />
          <DataRow label="Social media reviewed" value={data.socialMediaEvaluated} />
          {hasValue(data.linksToEvaluatedPages) && (
            <div style={{ padding: '10px 0' }}>
              <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--ink-light)', marginBottom: '4px' }}>Links</div>
              <p style={{ fontSize: '0.88rem', color: 'var(--ink-mid)', lineHeight: 1.6, wordBreak: 'break-all' }}>{data.linksToEvaluatedPages}</p>
            </div>
          )}
        </ProfileSection>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div style={{ background: 'var(--white)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }}>
          ...
          {hasValue(data.homepageUrl) && (
            <div style={{ padding: '10px 0', borderBottom: '0.5px solid var(--border-light)' }}>
              <a href={data.homepageUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.88rem', color: 'var(--red)', fontWeight: 600 }}>Visit homepage ↗</a>
            </div>
          )}
        </div>
      </div>
        ...
        <ProfileSection title="Admissions Page Content">
          ...
          {hasValue(admissions.keyPhrases) && (
            <div style={{ background: 'var(--parchment-mid)', borderRadius: 'var(--radius)', padding: '12px 14px', marginBottom: '1rem' }}>
              ...
              <p style={{ fontSize: '0.95rem', color: 'var(--navy)', lineHeight: 1.6 }}>{admissions.keyPhrases}</p>
            </div>
          )}
          ...
          {hasValue(admissions.pageUrl) && (
            <div style={{ padding: '10px 0' }}>
              <a href={admissions.pageUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.9rem', color: 'var(--red)', fontWeight: 600 }}>View admissions page ↗</a>
            </div>
          )}
        </ProfileSection>
      </div>
    ...
    <div className="profile-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem', alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <ProfileSection title="Student Life Analysis">
          ...
          <DataRow label="Headline strategy" value={data.headlineStrategy} />
          <DataRow label="Visual storytelling" value={data.visualStorytelling} />
          <DataRow label="Belonging & community focus" value={data.belongingCommunityFocus} />
          <DataRow label="Navigation & ease of use" value={data.navigationEaseOfUse} />
...
function AcademicsTab({ data }: { data: AcademicsAnalysis }) {
  return (
    <div className="profile-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem', alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {hasValue(data.strategicPositioningCoreMessaging) && (
          <NavyCard title="Strategic Positioning">
            <p style={{ fontSize: '1.05rem', color: 'var(--parchment)', lineHeight: 1.8 }}>{data.strategicPositioningCoreMessaging}</p>
          </NavyCard>
        )}
        {hasValue(data.notableFeaturesDifferentiators) && (
          <NavyCard title="Notable Differentiators" accent>
            <p style={{ fontSize: '1rem', color: 'var(--parchment)', lineHeight: 1.75 }}>{data.notableFeaturesDifferentiators}</p>
          </NavyCard>
        )}
  ...
  return (
    <div style={{ background: 'var(--navy)', borderRadius: 'var(--radius-lg)', padding: '1.5rem', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: '0.88rem', color: 'var(--ink-faint)', marginBottom: '1.25rem', lineHeight: 1.5 }}>
        Individual plan unlocks all content tabs plus CSV export.
      </p>
